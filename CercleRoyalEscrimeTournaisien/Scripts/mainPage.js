...
function changeTexteAutomatique() {
    switch ($("#CurrentVideo").val()) {
        ...
        case "2":
            ...
            if (CheckText(player.getCurrentTime(), 20, 38)) {
                $("#textVideo").html("Tout est valable. De la tête au pied.");
            }

            if (CheckText(player.getCurrentTime(), 39, 82)) {
            ...
            break;
        case "3":
            if (CheckText(player.getCurrentTime(), 0, 4)) {
                $("#textVideo").html("Le sabre<br>L' arme de cavalerie.");
            }

            if (CheckText(player.getCurrentTime(), 5, 28)) {
                $("#textVideo").html("La surface valable est tout ce qui est au dessus de la ceinture.");
            }

            if (CheckText(player.getCurrentTime(), 29, 33)) {
                $("#textVideo").html("Pour toucher, un effleurement de n'importe quelle partie de la lame suffit.");
            }

            if (CheckText(player.getCurrentTime(), 34, 62)) {
                $("#textVideo").html("Le premier à avoir 15 touches gagne le match.");
            }

            if (CheckText(player.getCurrentTime(), 63, 71)) {
                $("#textVideo").html("La passe avant est sanctionnée.");
            }

            if (CheckText(player.getCurrentTime(), 72, 77)) {
                $("#textVideo").html("Le premier qui touche n'a pas forcément le point!");
            }

            if (CheckText(player.getCurrentTime(), 78, 89)) {
                $("#textVideo").html("Celui qui a la priorité gagne le point.");
            }

            if (CheckText(player.getCurrentTime(), 90, 103)) {
                $("#textVideo").html("Celui qui attaque a la priorité.");
            }

            if (CheckText(player.getCurrentTime(), 104, 123)) {
                $("#textVideo").html("Mais si il rate sa touche, il perd la priorité. et la priorité est transférée à l'autre tireur qui aura le point si il touche.");
            }

            if (CheckText(player.getCurrentTime(), 124, 126)) {
                $("#textVideo").html("J'attaque j'ai priorité.");
            }

            if (CheckText(player.getCurrentTime(), 127, 139)) {
                $("#textVideo").html("Je pare l'attaque, je récupère la priorité. et j'ai droit à ma riposte. et je touche.");
            }
            break;
    }
}
...
function ActiveFonctionsGsm() {
    if (window.matchMedia("(max-width: 767px)").matches) {
        $("#tableYoutube").css('zoom', '0.3');


        $('*').each(function (i, ele) {
            if ($(ele).css('font-size') == '20px') { $(ele).css('font-size', '16px'); };
            if ($(ele).hasClass('classDarkGray')) { $(ele).css('font-size', '70px'); }
        });

        $(".classRedWhite ").eq(1).parent().css('left', '1%');
      
    }  
}

function ChangeVideo(step) {
    var data = $("#bodyMainPage :input").serializeArray();
    data.push({ name: "step", value: step });

    $.ajax({
        url: '/MainPage/ChangeVideo',
        type: 'POST',
        cache: false,
        data: data,
        dataType: 'json',
        ...
        success: function (result) {
            $('#bodyMainPage').html(result.mainPage);
...
function ShowMenuPrincipal() {
    window.location.href = '/MainPage/ShowMenuPrincipal/';
}

function ShowStartPage() {
    window.location.href = '/MainPage/StartPageNew/';
}