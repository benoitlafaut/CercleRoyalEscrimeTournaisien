...
namespace WebApplication1.Models
{
    public class Paiements_2025_2026
    {
        ...
        public void Add_Paiements_XXX_Amel(string period, List<MembreData> Membres)
        {
            if (!Membres.Any(x => x.GuidId == GuidConstantes.GuidAmelXXX && x.Période == period))
            {
                return;
            }
            Membres.Where(x => x.GuidId == GuidConstantes.GuidAmelXXX && x.Période == period).FirstOrDefault().Paiement =
                new Paiement()
                {
                    Periode = period,
                    IsCotisationAnnuelle = false,
